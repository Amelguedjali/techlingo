...
class _ChatRoomAIState extends State<ChatRoomAI> {
  ...
  void connect() {
    ...
    socket!.onConnect((_){
      ...
      socket!.on("oldmessages",(messages)=>{
        print(messages),
         for (var i = 0; i< messages.length; i++) {
          setState(() {
          listMsg.add( 
          MsgModel(msg: messages[i]["msg"], sender: messages[i]["sender"],send_time:messages[i]["send_time"], id_room: messages[i]["id_room"] ),
        );
        }),
      ...
      socket!.on("sendMsgServer", (msg) {
        ...
        setState(() {
          listMsg.add( 
          MsgModel(msg: msg["msg"], sender: msg["sender"],send_time:msg["send_time"], id_room: msg["id_room"] ),
        );
        });
        };
  ...
  void sendMsg(String msg, String sender) {
    ...
    MsgModel ownMsg = MsgModel(msg: msg,  sender: sender,  send_time: DateFormat.Hm().format(DateTime.now()), id_room: "AI");
    listMsg.add(ownMsg);
    setState(() {
      listMsg; 
    });
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index)  {
                  ...
                  if (listMsg[index].sender==widget.user_name){
                    return buildOwnMessagee(msg: listMsg[index].msg, sender: listMsg[index].sender, time: listMsg[index].send_time);
                  }
                  else{
                    return buildOtherMessagee(msg: listMsg[index].msg, sender: listMsg[index].sender, time: listMsg[index].send_time);
                  }
                },
